...
check_claude_version() {
    ...
    if [[ ! -f "$settings_file" ]]; then
        ...
        warning "Claude Code v1.0.41 이상이 필요합니다. (PreCompact hook 지원)"
        echo -n "계속하시겠습니까? (y/N): "
        read -r response
        if [[ ! "$response" =~ ^[Yy]$ ]]; then
            error "설치가 취소되었습니다."
...
do_uninstall() {
    ...
    # 토큰 모니터링 관련 디렉토리 삭제 확인
    if [[ -d "$HISTORY_DIR" ]] || [[ -d "$SUMMARY_DIR" ]]; then
        echo -n "대화 기록 및 요약 디렉토리도 삭제하시겠습니까? (y/N): "
        read -r response
        if [[ "$response" =~ ^[Yy]$ ]]; then
            [[ -d "$HISTORY_DIR" ]] && rm -rf "$HISTORY_DIR" && info "대화 기록이 삭제되었습니다."
            [[ -d "$SUMMARY_DIR" ]] && rm -rf "$SUMMARY_DIR" && info "요약 기록이 삭제되었습니다."
        fi
    fi
    
    # 로그 파일 삭제 확인
    local log_files=(
        "$LOG_DIR/claude_md_injector.log"
        "$LOG_DIR/claude_md_precompact.log"
        "$LOG_DIR/claude_token_monitor.log"
        "$LOG_DIR/claude_enhanced_precompact.log"
    )
    local logs_exist=false
    for log_file in "${log_files[@]}"; do
        [[ -f "$log_file" ]] && logs_exist=true && break
    done
    
    if [[ "$logs_exist" == true ]]; then
        echo -n "로그 파일들도 삭제하시겠습니까? (y/N): "
        read -r response
        if [[ "$response" =~ ^[Yy]$ ]]; then
            for log_file in "${log_files[@]}"; do
